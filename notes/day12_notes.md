...
```mermaid
flowchart TD
  A[User Goal] --> B[Sense<br/>(parse goal, context)]
  B --> C[Plan<br/>(decompose, pick tools)]
  C --> D[Retrieve (iterative)<br/>(search, re‑rank, expand)]
  D --> E[Reason<br/>(synthesize evidence)]
  E --> F{Verify OK?}
  F -- no --> C
